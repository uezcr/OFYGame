...
#include "UI/Frontend/OFYFrontendStateComponent.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(OFYFrontendStateComponent)
...
void UOFYFrontendStateComponent::BeginPlay()
{
	Super::BeginPlay();
	...
	ExperienceComponent->CallOrRegister_OnExperienceLoaded_HighPriority(FOnOFYExperienceLoaded::FDelegate::CreateUObject(this, &ThisClass::OnExperienceLoaded));
}

void UOFYFrontendStateComponent::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	Super::EndPlay(EndPlayReason);
}

bool UOFYFrontendStateComponent::ShouldShowLoadingScreen(FString& OutReason) const
{
...
void UOFYFrontendStateComponent::OnExperienceLoaded(const UOFYExperienceDefinition* Experience)
{
...
void UOFYFrontendStateComponent::OnUserInitialized(const UCommonUserInfo* UserInfo, bool bSuccess, FText Error,
	ECommonUserPrivilege RequestedPrivilege, ECommonUserOnlineContext OnlineContext)
{
	...
	{
		UserSubsystem->OnUserInitializeComplete.RemoveDynamic(this, &UOFYFrontendStateComponent::OnUserInitialized);
...
void UOFYFrontendStateComponent::FlowStep_WaitForUserInitialization(FControlFlowNodeRef SubFlow)
{
...
void UOFYFrontendStateComponent::FlowStep_TryShowPressStartScreen(FControlFlowNodeRef SubFlow)
{
	...
	if (!UserSubsystem->ShouldWaitForStartInput())
	{
		...
		UserSubsystem->OnUserInitializeComplete.AddDynamic(this, &UOFYFrontendStateComponent::OnUserInitialized);
...
void UOFYFrontendStateComponent::FlowStep_TryJoinRequestedSession(FControlFlowNodeRef SubFlow)
{
...
void UOFYFrontendStateComponent::FlowStep_TryShowMainScreen(FControlFlowNodeRef SubFlow)
{
	if (UPrimaryGameLayout* RootLayout = UPrimaryGameLayout::GetPrimaryGameLayoutForPrimaryPlayer(this))
	{
		constexpr bool bSuspendInputUntilComplete = true;
