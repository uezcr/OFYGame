...
#include "Camera/OFYCameraMode_ThirdPerson.h"

#include "OFYCameraAssistInterface.h"
#include "Curves/CurveVector.h"
#include "Engine/Canvas.h"
#include "GameFramework/CameraBlockingVolume.h"
#include "GameFramework/Character.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(OFYCameraMode_ThirdPerson)

namespace OFYCameraMode_ThirdPerson_Statics
{
	static const FName NAME_IgnoreCameraCollision = TEXT("IgnoreCameraCollision");
}

UOFYCameraMode_ThirdPerson::UOFYCameraMode_ThirdPerson()
{
	TargetOffsetCurve = nullptr;

	PenetrationAvoidanceFeelers.Add(FOFYPenetrationAvoidanceFeeler(FRotator(+00.0f, +00.0f, 0.0f), 1.00f, 1.00f, 14.f, 0));
	PenetrationAvoidanceFeelers.Add(FOFYPenetrationAvoidanceFeeler(FRotator(+00.0f, +16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
	PenetrationAvoidanceFeelers.Add(FOFYPenetrationAvoidanceFeeler(FRotator(+00.0f, -16.0f, 0.0f), 0.75f, 0.75f, 00.f, 3));
	PenetrationAvoidanceFeelers.Add(FOFYPenetrationAvoidanceFeeler(FRotator(+00.0f, +32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
	PenetrationAvoidanceFeelers.Add(FOFYPenetrationAvoidanceFeeler(FRotator(+00.0f, -32.0f, 0.0f), 0.50f, 0.50f, 00.f, 5));
	PenetrationAvoidanceFeelers.Add(FOFYPenetrationAvoidanceFeeler(FRotator(+20.0f, +00.0f, 0.0f), 1.00f, 1.00f, 00.f, 4));
	PenetrationAvoidanceFeelers.Add(FOFYPenetrationAvoidanceFeeler(FRotator(-20.0f, +00.0f, 0.0f), 0.50f, 0.50f, 00.f, 4));
}

void UOFYCameraMode_ThirdPerson::UpdateView(float DeltaTime)
{
...
void UOFYCameraMode_ThirdPerson::UpdateForTarget(float DeltaTime)
{
...
void UOFYCameraMode_ThirdPerson::UpdatePreventPenetration(float DeltaTime)
{
	...
	IOFYCameraAssistInterface* TargetControllerAssist = Cast<IOFYCameraAssistInterface>(TargetController);

	IOFYCameraAssistInterface* TargetActorAssist = Cast<IOFYCameraAssistInterface>(TargetActor);
	...
	IOFYCameraAssistInterface* PPActorAssist = OptionalPPTarget.IsSet() ? Cast<IOFYCameraAssistInterface>(PPActor) : nullptr;
	...
	if (PPActorRootComponent)
	{
		...
		IOFYCameraAssistInterface* AssistArray[] = { TargetControllerAssist, TargetActorAssist, PPActorAssist };

		if (AimLineToDesiredPosBlockedPct < ReportPenetrationPercent)
		{
			for (IOFYCameraAssistInterface* Assist : AssistArray)
			{
				if (Assist)
				{
...
void UOFYCameraMode_ThirdPerson::PreventCameraPenetration(AActor const& ViewTarget, FVector const& SafeLoc,
	FVector& CameraLoc, float const& DeltaTime, float& DistBlockedPct, bool bSingleRayOnly)
{
	...
	{
		FOFYPenetrationAvoidanceFeeler& Feeler = PenetrationAvoidanceFeelers[RayIdx];
		if (Feeler.FramesUntilNextTrace <= 0)
		{
			...
			if (bHit && HitActor)
			{
				...
				if (HitActor->ActorHasTag(OFYCameraMode_ThirdPerson_Statics::NAME_IgnoreCameraCollision))
				{
					bIgnoreHit = true;
...
void UOFYCameraMode_ThirdPerson::DrawDebug(UCanvas* Canvas) const
{
...
void UOFYCameraMode_ThirdPerson::SetTargetCrouchOffset(FVector NewTargetOffset)
{
...
void UOFYCameraMode_ThirdPerson::UpdateCrouchOffset(float DeltaTime)
{
